...
## Git 推送时遇到 `gnutls_handshake() failed` 错误的完整记录。

**问题描述**：在执行 `git push origin main` 时出现以下错误：

```bash
fatal: unable to access 'https://github.com/Delluna/idea1.git/':  gnutls_handshake() failed: Error in the pull function.
```

**最终解决方案**：完全绕过 HTTPS + 代理问题，改用 **SSH 协议**连接 GitHub

  - **1️⃣ 生成 SSH 密钥（如果尚未生成）**：
    ```bash
    ssh-keygen -t ed25519 -C "你的邮箱@example.com"
    # 一路按回车（使用默认路径，可不设密码短语）
    ```
...
# 查看当前配置确认
git config --global --list | grep -E "(proxy|sslBackend)"
```
